...
SOCKET_PATH=${LNHV1_SOCKET_PATH:-"/tmp/linux_native_hook_v1_tracking_optimization_$$.sock"}
...
CSV_PATH=${LNHV1_CSV_PATH:-"${ROOT_DIR}/results/hook_tracking_optimization_server_$(date +%F).csv"}
...
run_full_reference()
{
    ...
    consumer_log="${ROOT_DIR}/results/logs/tracking_optimization_stage${stage}_${stage_name_value}_${tracking_mode}_cache1_t${threads}_s${size}_d${DURATION}.consumer.log"
...
for threads in "${THREAD_VALUES[@]}"; do
    for size in "${SIZE_VALUES[@]}"; do
        ...
        for tracking_mode in global sharded; do
