...
SOCKET_PATH=${LNHV1_SOCKET_PATH:-"/tmp/linux_native_hook_v1_record_write_ablation_$$.sock"}
...
SUB_STAGE_LIST=${LNHV1_SUBABLATION_STAGE_LIST:-5,6,7,8,9,10,11}
CSV_PATH=${LNHV1_CSV_PATH:-"${ROOT_DIR}/results/hook_record_write_subablation_server_$(date +%F).csv"}
...
sub_stage_name()
{
    case "$1" in
        ...
        5) printf 'record_fill_minimal' ;;
        6) printf 'metadata_clock' ;;
        7) printf 'metadata_pid_tid' ;;
        8) printf 'metadata_thread_name' ;;
        9) printf 'ring_index_check' ;;
        10) printf 'shm_record_copy' ;;
        11) printf 'atomic_index_self_drain' ;;
        *) printf 'unknown' ;;
    esac
}
...
for threads in "${THREAD_VALUES[@]}"; do
    for size in "${SIZE_VALUES[@]}"; do
        ...
        full_log="${ROOT_DIR}/results/logs/record_write_full_t${threads}_s${size}_d${DURATION}.consumer.log"
        ...
        for sub_stage in "${SUB_STAGE_VALUES[@]}"; do
            ...
            if [[ "${name}" == "unknown" ]]; then
                echo "invalid record-write sub-ablation stage: ${sub_stage}" >&2
                exit 1
            fi

            log_suffix="record_write_substage${sub_stage}_${name}_t${threads}_s${size}_d${DURATION}"
