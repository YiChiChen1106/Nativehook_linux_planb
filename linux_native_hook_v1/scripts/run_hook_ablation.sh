...
SOCKET_PATH=${LNHV1_SOCKET_PATH:-"/tmp/linux_native_hook_v1_ablation_$$.sock"}
...
DURATION=${LNHV1_DURATION:-3}
STAGE_LIST=${LNHV1_ABLATION_STAGE_LIST:-1,2,3,4,5,6}
CSV_PATH=${LNHV1_CSV_PATH:-"${ROOT_DIR}/results/hook_ablation_$(date +%F).csv"}
...
run_hooked_workload()
{
    ...
    LNHV1_ABLATION_STAGE="${stage}" LNHV1_SOCKET_PATH="${SOCKET_PATH}" LD_PRELOAD="${HOOK_LIB}" \
        "${WORKLOAD}" --threads "${threads}" --duration "${duration}" --size "${size}"
}
...
cat >"${CSV_PATH}" <<'EOF'
stage,stage_name,threads,size,duration,flush_threshold,sample_interval,filter_size,blocked,baseline_ops,with_hook_ops,overhead_pct,records,alloc,free,thread_name,flush,dropped,avg_batch,consumer_log
EOF
...
for threads in "${THREAD_VALUES[@]}"; do
    for size in "${SIZE_VALUES[@]}"; do
        ...
        for stage in "${STAGE_VALUES[@]}"; do
            ...
            log_suffix="ablation_stage${stage}_${name}_t${threads}_s${size}_d${DURATION}"
            ...
            hook_output=$(run_hooked_workload "${stage}" "${threads}" "${DURATION}" "${size}")
            hook_line=$(printf '%s\n' "${hook_output}" | tail -n1)
            ...
            printf '[stage %s %s] threads=%s size=%s baseline_ops=%s with_hook_ops=%s overhead_pct=%s records=%s flush=%s dropped=%s avg_batch=%s\n' \
                "${stage}" \
                "${name}" \
                "${threads}" \
                "${size}" \
