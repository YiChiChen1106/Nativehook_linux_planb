...
function hdr(slide, num, title, sub) {
  ...
  slide.addText(String(num).padStart(2, "0"), {
    ...
    fontSize: 24, bold: true, color: C.blue, fontFace: "Consolas", align: "right",
  });
  slide.addText(title, {
    x: 1.4, y: 0.15, w: 10, h: 0.45,
    fontSize: 20, bold: true, color: C.ink,
  });
  if (sub) slide.addText(sub, {
    x: 1.4, y: 0.58, w: 10, h: 0.25,
...
function tbl(slide, rows, x, y, w) {
  ...
  const hdrRow = rows[0].map((h, i) => ({
    text: h, options: { bold: true, fontSize: 10, color: C.blue, fill: { color: C.blueBg }, align: i === 0 ? "left" : "center" }
  }));
  const datRows = rows.slice(1).map((r, ri) =>
    r.map((c, ci) => ({
      text: String(c), options: {
        fontSize: 10, color: ci === n - 1 ? C.green : C.dim,
  ...
  slide.addTable([hdrRow, ...datRows], {
    ...
    colW: Array(n).fill(cw), rowH: 0.36, margin: [2, 6, 2, 6],
  });
}

function blt(slide, lines, x, y, w, h, fs) {
  slide.addText(lines.map(t => ({
    text: t, options: { bullet: { code: "2022" }, fontSize: fs || 12, color: C.dim, breakType: "none", paraSpaceAfter: 4 }
  })), {
...
function big(slide, val, sub, x, y) {
  slide.addText(val, { x, y, w: 2.5, h: 0.8, fontSize: 40, bold: true, color: C.blue, align: "center", fontFace: "Consolas" });
  slide.addText(sub, { x, y: y + 0.75, w: 2.5, h: 0.3, fontSize: 9, color: C.faint, align: "center" });
}
...
(() => {
  ...
  s.addText("native_hook", {
    ...
    fontSize: 64, bold: true, color: C.ink, align: "center",
  });
  s.addText("Producer Hot Path 优化进展", {
    x: 1, y: 2.95, w: 11, h: 0.6,
  ...
  s.addText("2026.06.05  组会汇报  ·  陈一驰", {
    ...
    fontSize: 13, color: C.faint, align: "center",
  });
})();
...
(() => {
  ...
  [
    ...
    s.addText(o.n, { x: 0.65, y: y + 0.2, w: 0.7, h: 0.7, fontSize: 26, bold: true, color: o.c, fontFace: "Consolas" });
    s.addText(o.t, { x: 1.5, y: y + 0.15, w: 6, h: 0.35, fontSize: 13, bold: true, color: C.ink });
...
(() => {
  ...
  s.addText("失败的优化", { x: 0.55, y: 1.5, w: 4, h: 0.3, fontSize: 13, bold: true, color: C.pink });
  blt(s, [
    "PID/TID cache",
  ...
  s.addText("三步预检法  [AGENTS.md]", { x: 6.7, y: 1.5, w: 5, h: 0.3, fontSize: 13, bold: true, color: C.green });
  blt(s, [
    "①  画映射表",
    "    原型每个模块 ↔ 真实代码 函数+行号",
    "",
...
(() => {
  ...
  [
    ...
    s.addText(it.n, { x: 1.35, y: y + 0.15, w: 0.8, h: 0.8, fontSize: 30, bold: true, color: it.c, fontFace: "Consolas" });
    s.addText(it.t, { x: 2.3, y: y + 0.2, w: 5, h: 0.35, fontSize: 14, bold: true, color: C.ink });
    s.addText(it.d, { x: 2.3, y: y + 0.65, w: 9, h: 0.5, fontSize: 11, color: C.dim });
  });
})();
